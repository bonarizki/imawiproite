...
use App\Model\User;
use App\Model\Appraisal\Access;
...
class AccessController extends Controller
{
    public function index()
    {
        ...
        return view('appraisal.master.access', compact('parent', 'child', 'grand_child'));
    }
    ...
    public function update(Request $request)
    {
        ...
        DB::transaction(function() use ($request, $menu) {
            ...
            \Helper::instance()->log('UPDATE', $request, 'App\Model\Appraisal\Access');
    ...
    {
    	$access = Access::query()
            ->selectRaw('mst_appraisal_access.access_id,
                mst_appraisal_access.user_id,
                CONCAT("[", user.user_nik, "] - ", user.user_name) AS user,
                dept.department_name,
                mst_appraisal_access.menu')
            ->join('mst_main_user AS user', 'user.user_id', 'mst_appraisal_access.user_id')
