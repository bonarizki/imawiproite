...
use App\Model\Appraisal\AccessPosition;
...
class AccessPositionController extends Controller
{
    public function index()
    {
        ...
        return view('appraisal.master.access_position', compact('department', 'parent', 'child', 'grand_child'));
    }

    public function store(Request $request)
    {
        $rules = [
            'department_id' => 'required|unique:mst_appraisal_access_position,department_id'
        ];
        ...
        DB::transaction(function() use ($request, $module) {
            ...
            \Helper::instance()->log('CREATE', $request, 'App\Model\Appraisal\AccessPosition');
    ...
    public function update(Request $request)
    {
        ...
        DB::transaction(function() use ($request, $module) {
            ...
            \Helper::instance()->log('UPDATE', $request, 'App\Model\Appraisal\AccessPosition');
    ...
    public function getAccessPosition(Request $request)
    {
    	$access = AccessPosition::query()
            ->selectRaw('mst_appraisal_access_position.access_position_id,
                mst_appraisal_access_position.department_id,
                dept.department_name,
                mst_appraisal_access_position.menu')
            ->leftJoin('mst_main_department AS dept', 'dept.department_id', 'mst_appraisal_access_position.department_id')
            ->orderBy('dept.department_id')
        ->get();

        return datatables()->of($access)->addIndexColumn()
            ->addColumn('view', function($access) {
                return '<a href="javascript:;" class="btn-view" data-dept="'.$access->department_name.'" data-mod="'.$access->menu.'" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>';
            })
            ->addColumn('edit', function($access) {
                return '<a href="javascript:;" class="btn-edit" data-id="'.$access->access_position_id.'" data-dept_id="'.$access->department_id.'" data-dept="'.$access->department_name.'" data-mod="'.$access->menu.'" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i></a>';
            })
