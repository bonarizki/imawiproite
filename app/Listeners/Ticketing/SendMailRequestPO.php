...
use App\Events\Ticketing\SendMailRequestPO AS SendMailEvenet;
use App\Mail\MailTicketing\MailRequestPo;
...
class SendMailRequestPO
{
     ...
     * @return void
     */
    public function handle(SendMailEvenet $event)
    {
        ...
        if ($user_nik == '72231706') {// jika yang approve pak charles maka akan email ke deka
            $approval_nik = '75832003'; // NIk deka hrga
        }else{
        ...
        if ($approval_nik != null) {
            ...
            try {
                \Mail::to(['address' => $receiver->user_email])
                    ...
                    ->send(new MailRequestPo($header,$detail,$receiver));
            } catch (\Throwable $th) {
