/*!
 * knockout-monthrangepicker
 * version: 0.1.0
 * authors: Sensor Tower team
 * license: MIT
 * https://sensortower.github.io/monthrangepicker
 */
(function() {
  ...
  $.fn.monthrangepicker = function(options, callback) {
    ...
    this.each(function() {
      var $element;
      $element = $(this);
      if (!$element.data('monthrangepicker')) {
        options.anchorElement = $element;
        if (callback) {
          options.callback = callback;
        }
        options.callback = $.proxy(options.callback, this);
        return $element.data('monthrangepicker', new DateRangePickerView(options));
      }
    });
    return this;
  };
  ...
  ko.bindingHandlers.monthrangepicker = (function() {
    return $.extend(this, {
      _optionsKey: 'monthrangepickerOptions',
      _formatKey: 'monthrangepickerFormat',
      init: function(element, valueAccessor, allBindings) {
        var observable, options;
        observable = valueAccessor();
        options = ko.unwrap(allBindings.get(this._optionsKey)) || {};
        return $(element).monthrangepicker(options, function(startDate, endDate, period) {
      ...
      update: function(element, valueAccessor, allBindings) {
        ...
        return ko.ignoreDependencies(function() {
          var text;
          if (!$element.data('monthrangepicker').standalone()) {
            text = $element.data('monthrangepicker').single() ? startDateText : startDateText + " – " + endDateText;
            $element.val(text).text(text);
          }
          $element.data('monthrangepicker').startDate(startDate);
          return $element.data('monthrangepicker').endDate(endDate);
        });
      }
    });
  })();
  ...
  DateRangePickerView = (function() {
    function DateRangePickerView(options) {
      ...
      if (this.anchorElement) {
        ...
        if (!this.standalone()) {
          $(document).on('mousedown.monthrangepicker', this.outsideClick).on('touchend.monthrangepicker', this.outsideClick).on('click.monthrangepicker', '[data-toggle=dropdown]', this.outsideClick).on('focusin.monthrangepicker', this.outsideClick);
        }
      }
      if (this.opened()) {
        this.updatePosition();
  ...
  DateRangePickerView.template = ''+
    '<div class="monthrangepicker" data-bind="css: $data.cssClasses(), style: $data.style()">'+
      ...
      '<div class="controls">'+
        ...
        '<form data-bind="submit: $data.applyChanges">'+
          ...
          '<div class="custom-range-buttons">'+
            ...
            '<button class="cancel-btn hide" data-bind="text: $data.locale.cancelButtonTitle, click: $data.cancelChanges"></button>'+
          '</div>'+
        '</form>'+
      '</div>'+
    '</div>';

  $.extend($.fn.monthrangepicker, {
