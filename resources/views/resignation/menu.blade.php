...
@section('script')
    <script>
            ...
            $('#table tbody').on('click', 'td.details-control', function () {
                ...
                if (row.child.isShown()) {
                    ...
                    var table_child = $(`#table_child_${id}`).DataTable({
                        ...
                        sDom: 'lrtip',
                        destroy: true,
                        ajax: "{{url('/resignation/all/menuChild')}}/" + id,
                        columns: [{
                ...
                $(`#table_child_${row.data().menu_parent_id} tbody`).on('click', 'td.details-control-child', function () {
                    ...
                    if (row.child.isShown()) {
                        ...
                        $(`#table_grand_child_${id}`).DataTable({
                            ...
                            destroy: true,
                            sDom: 'lrtip',
                            ajax: "{{url('/resignation/all/menuGrandChild')}}/" + id,
                            columns: [
                                {
                                    data: 'DT_RowIndex',
                                    name: 'DT_RowIndex'
        ...
        function prosesDelete(typeMenu, id) {
            $.ajax({
                type: "delete",
                url: "{{url('/resignation/delete/menu')}}",
                data: {
        ...
        function modalEdit(type, typeParameter, id) {
            $.ajax({
                asyn: false,
                type: "get",
                url: "{{url('/resignation/detail/menu')}}/",
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        function updateData(typeMenu) {
            ...
            $.ajax({
                type: "PUT",
                url: "{{url('/resignation/update/menu')}}",
                data: data,
                success: function (response) {
                    sweetSuccess(response.status, response.message);
                    refreshTables(typeMenu)
        ...
        function optionChild(d) {
            ...
            if (id != '') {
                $.ajax({
                    url: "{{url('/resignation/all/menuChild')}}/" + id,
                    type: "GET",
                    success: function (response) {
                        let data = response.data;
                        let option = '';
        ...
        function insert(typeMenu) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('/resignation/insert/menu')}}",
                data: data,
                success: function (response) {
                    refreshTables(typeMenu)
                    closeModal();
