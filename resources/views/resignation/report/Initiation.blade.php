...
@section('breadcumb','Attrition Rate Based On Initiation')
@section('title','Attrition Rate Initiation')
...
<script>
    ...
    function viewDataPeriod()
    {
        ...
        $.ajax({
            type:"post",
            data: data,
            url:"{{url('resignation/report/AR/initiation/data')}}",
    ...
    function downloadByPeriod() {
        let data = $('#form').serialize();
        data += `&type=period`;
        window.open("{{url('resignation/report/AR/initiation/download')}}?"+data , '_blank');
    }
    ...
    function loopingTable(data)
    {
        ...
        for (let index = 0; index < data.length; index++) {
            tableBody += `<tr>
                            ...
                            <td>${data[index].total_volunteer}</td>
                            <td>${data[index].total_non_volunteer}</td>
                        </tr>`;
        }
