...
<head>
    <title>I'am A Wiproite | Training - @yield('title')</title>
    ...
    @include('training/master/include/header')
    <style>
        .se-pre-con {
            position: fixed;
            left: 0px;
...
<body class="horizontal-layout horizontal-menu 2-columns  navbar-floating footer-static " data-open="hover" data-menu="horizontal-menu" data-col="2-columns">
        ...
        <div class="header-navbar navbar-expand-sm navbar navbar-horizontal floating-nav navbar-light navbar-without-dd-arrow navbar-shadow menu-border" role="navigation" data-menu="menu-wrapper">
            <div class="navbar-header">
                <ul class="nav navbar-nav flex-row">
                    <li class="nav-item mr-auto"><a class="navbar-brand">
                            <h2 class="brand-text mb-0">TRAINING</h2>
                        </a>
                    </li>
            ...
            <div class="navbar-container main-menu-content" data-menu="menu-container">
                ...
                <ul class="nav navbar-nav hover" id="main-menu-navigation" data-menu="menu-navigation">
                    @php
                        $allMenu = App\Model\Training\Menu\Menu::with(['MenuChild','MenuChild.MenuGrandChild'])->get();
                        $access = App\Model\Training\Access::where('user_id', Auth::user()->user_id)->first();
                        if ($access) {
                                ...
                                <li class="dropdown nav-item parent-{{$parentMenu->menu_parent_id}}" data-menu="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        @foreach ($parentMenu->MenuChild as $menuChild)
                                            @if (in_array($menuChild->menu_child_id, $child) )
                                                @if (count($menuChild->MenuGrandChild)>0)
                                                    <li class="dropdown dropdown-submenu" data-menu="dropdown-submenu"><a class="dropdown-item dropdown-toggle" href="{{url($menuChild->menu_child_url)}}" data-toggle="dropdown"><i class="{{$menuChild->menu_child_icon}}"></i>{{$menuChild->menu_child_name}}</a>
                                                        ...
                                                        @foreach ( $menuChild->MenuGrandChild as $menuGrandChild)
                                                            @if (in_array($menuGrandChild->menu_grand_child_id, $grand_child))
                                                                <li data-menu="">
    ...
    @include('training/master/include/footer')
    <script>
        ...
        getBreadcum = () => {
            ...
            $.ajax({
                type: "get",
                url: "{{url('breadcumTraining')}}",
                data: {
                    path: path
                },
                success: function (response) {
                    let data = response.data;
                    let breadcumb = '<li class="breadcrumb-item"><a href="{{url('/training')}}">Home</a></li>'
                    if (path != 'training') {
                        if (data != null) {
