...
class RecaptchaV3
{
    ...
    {
        return $this->origin . '/api.js?render=' . $this->sitekey;
    }
    ...
    public function fieldCustomBonBon($action, $name = 'g-recaptcha-response')
    {
        $fieldId = uniqid($name . '-', false);
        $html = '<input type="hidden" name="' . $name . '" id="' . $name . '" class="' . $name . '">';
        return $html;
    }

    public function scriptBonBon($action, $name = 'g-recaptcha-response')
    {
        $fieldId = uniqid($name . '-', false);
